...
class ModelExecutor:
    ...
    def create_placeholders(self):
        ...
        self.x = tf.compat.v1.placeholder(tf.float32, (None, input_dims[0], input_dims[1], input_dims[2]),
    ...
    def train_model(self, X_train_features, X_train_labels, X_valid_features, y_valid_labels, 
        ...
        with tf.compat.v1.Session(graph=self.graph, config=tf.compat.v1.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
            ...
            X_train_features = X_train_features.numpy()
            X_train_labels = X_train_labels.numpy()
...
def EdLeNet(x, mc, dropout_conv_pct, dropout_fc_pct):
    ...
    for i in range(0, mc.conv_layers_count):
        ...
        conv_W = tf.Variable(tf.random.truncated_normal(shape=(mc.conv_filter_size, mc.conv_filter_size, conv_input_depth, conv_output_depth), 
        ...
        conv_output = tf.nn.max_pool2d(conv_output, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
    ...
    for output_dim in mc.fc_output_dims:
        fcn_W  = tf.Variable(tf.compat.v1.truncated_normal(shape=(prev_layer.get_shape().as_list()[-1], output_dim), 
                                                 mean = mu, stddev = sigma))
    ...
    fc_final_W  = tf.Variable(tf.compat.v1.truncated_normal(shape=(prev_layer.get_shape().as_list()[-1], mc.output_classes), 
                                                  mean = mu, stddev = sigma))
...
def main():
    ...
    print("Type of valid_features: ")
    print(type(valid_features))
    print("Type of valid_labels: ")
    print(type(valid_labels))

    print("Type of test_features: ")
